...
const agregaAlimentos = () => {
    ...
    form.addEventListener("submit", function (event) {
        ...
        const mesa = parseInt(datos.get("mesa"));
        const alimento = datos.get("alimento");
        const cantidad = parseInt(datos.get("cantidad"));

        if (!mesa || !alimento || !cantidad) {
            alert("Tienes opciones sin completar");
            return;
        }

        // // Guardar en DB
        // guardarPedido(mesa, alimento, cantidad);

        // // Actualizar la tabla
        // pedidos.push({ mesa, alimento, cantidad });
        // tablaDePedido(pedidos);

        window.api.guardarPedido(mesa, alimento, cantidad); // <- cambio clave

        tablaDePedido(pedidos);
        //tablaDePedido([{ mesa, alimento, cantidad }]);
...
document.addEventListener("DOMContentLoaded", () => {
    agregaAlimentos();
    cargarPedidos(); // <-- esta línea nueva
});